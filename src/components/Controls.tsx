import { Box as BoxIcon, Layers, Plus, Truck, Weight, X } from 'lucide-react';
...
export function Controls({
  ...
  return (
    <div className="rounded-lg border border-navy/10 bg-canvas-card p-4 shadow-card">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-[1.3fr_1.7fr_1fr]">
        <div className="sm:col-span-2 lg:col-span-1">
          <Field label="Pallet size" icon={<Truck size={14} />}>
            <select
              value={palletId}
              onChange={(e) =>
                onPalletChange(e.target.value as PalletStandardId)
              }
        ...
        <Field label="Box weight" icon={<Weight size={14} />}>
          <div className="grid grid-cols-2 gap-2">
            <NumInput
              value={box.weightLbs}
              min={0}
              step={1}
              onChange={(v) => onBoxChange({ ...box, weightLbs: v })}
              suffix="lbs"
              ariaLabel="Box weight in pounds"
            />
            <NumInput
              value={box.weightOz}
              min={0}
              max={15}
              step={1}
              onChange={(v) => onBoxChange({ ...box, weightOz: v })}
              suffix="oz"
              ariaLabel="Box weight in ounces"
            />
          </div>
        </Field>
      </div>
...
function NumInput({
  ...
  const paddingRight =
    suffix && suffix.length > 1 ? 'pr-10' : suffix ? 'pr-7' : 'pr-3';
    ...
    <div className="relative">
      <input
        ...
        onChange={(e) => {
          ...
          let val = integer ? Math.round(raw) : raw;
          if (min !== undefined) {
        ...
        className={`w-full rounded-md border border-navy/15 bg-white px-3 py-2 ${paddingRight} font-medium text-navy-ink text-sm focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/20`}
      />
      {suffix ? (
